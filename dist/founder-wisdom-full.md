...
- Paying the small, sympathetic vendors first on the way down. It feels like decency, it's an out-of-order payment once you're insolvent, and a trustee can claw it back out of the vendor's hands — the claim against you is for making it.
...
**A founder whose immigration status runs through the company is on a clock the company controls.** When status is employer-sponsored, ending the employment relationship opens a grace period set by federal rule. It does not apply to every classification. Where it does, it runs about two months at the outside — shorter if your authorized stay ends sooner, available only once per validity period, and discretionary besides. For some founders it is effectively zero; a green-card process tied to this employer can go with the job. Material changes to the role, the ownership, or the entity can require a new filing, so the restructuring that solves the business problem can create an immigration one. Get immigration counsel in the room while there is still runway to sequence around it; the clock runs from the end of employment, not from the day you start looking.

**Immigration makes co-founder decisions asymmetric — the wind-down that costs one founder a hard year costs the other the country.** The grace period above is the mechanic; the asymmetry is what it does to the two of you. The co-founder who wants out cannot simply leave. And whoever holds the power to terminate holds an immigration consequence over the other, which the other is carrying silently through every equity conversation the two of you have. Neither of you chose that leverage and neither can put it down, so name it out loud while the decisions are still hypothetical.
