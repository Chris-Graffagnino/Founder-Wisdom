...
- **`dist/system-prompt.md`** — a one-page system-prompt stub for platforms where the corpus lives in a retrieval store rather than in context (e.g. a ChatGPT Custom GPT with the reference files uploaded as knowledge). It carries the trigger conditions, the two modes, the output-style rules, and an instruction to retrieve whole topic sections by name to compensate for chunk-based retrieval.
...
The script is deterministic and fails loudly if the sources drift out of sync with its rewrites (e.g. a reference file missing from SKILL.md's routing list).
