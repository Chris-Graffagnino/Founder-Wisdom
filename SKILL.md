...
- `references/governance.md` — Boards, independent directors, board communication, Reid Hoffman's three-light framework, the *Startup Boards* approach to board leadership (onboard a director like an executive, one written statement of expectations covering every seat, periodic feedback on the board and each member — Feld, Blumberg & Ramsinghani, with the expectations one-pager and the peer survey drawn from Feld's blog and Blumberg's Bolster, not the book), Fred Wilson on board design — investor seats arrive by contract, so the independent seats are the only ones you actually get to design — and Wasserman on succession: shipping the product and closing a round each raise a founder-CEO's odds of being replaced
